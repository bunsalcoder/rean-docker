...
អានរបាយការណ៍; កុំភ័យនឹង «LOW» គ្រប់មួយ។ ចំណុចគឺ៖ **ដឹងរបៀបស្កេន** មុន promote image។ ជម្រើសផ្សេង៖ Docker Scout (`docker scout` លើ Docker Desktop ឬ Scout CLI បើមាន), Grype ឬ scanner ណាដែល org របស់អ្នកប្រើ — curriculum នេះស្តង់ដារ **Trivy** ក្នុង CI ដើម្បីឱ្យ gate ចម្លងបានដោយមិនពឹង Desktop។

CI របស់ repo នេះស្កេន teaching builds ជាមួយ Trivy៖ **HIGH** ជារបាយការណ៍ ហើយ **unfixed CRITICAL** fail job សម្រាប់ Labs **05, 08 (slim), 09, 12, និង 13**។ Builds ដែល **ខ្សោយ/ដំបូងដោយចេតនា** (Lab 03, Lab 08 fat image, Lab 04 leaky `ENV`, Lab 11 secret demo) នៅតែ compile ក្នុង CI ប៉ុន្តែ **មិន** នៅលើ CRITICAL gate — កុំមើល pipeline បៃតងថាជា “Dockerfile គ្រប់មួយ production-clean”។ Runtime Dockerfiles លុប `npm`/`corepack` ពី base image បន្ទាប់ពី `npm ci` ដើម្បីឱ្យ gate ផ្តោតលើអ្វីដែល app ផ្ញើ — មិនមែន CVE គ្រប់មួយក្នុង package manager របស់ Node (ពិភាក្សា Lab 09)។
...
Tags ផ្លាស់ទី។ Digest (`alpine@sha256:…`) គឺ bits ដែលអ្នកពិតជា pull។ Pin digests ពេលគ្រប់គ្រង supply-chain សំខាន់ (CI, production)។ Labs 05, 08 (slim), 09, 12, និង 13 pin `FROM node:22-alpine@sha256:…`; Lab 13 pin Postgres/Redis ក្នុង Compose ដែរ។ Digest ហួសសម័យដោយចេតនា — refresh Dockerfiles តាម Dependabot ហើយ Lab 13 Compose pins តាម `make refresh-digests` (CI weekly workflow រត់ target នោះ ហើយបើក PR ទៅ `develop` ពេល pin drift; optionally រត់ `make refresh-digests-check` locally ដើម្បី fail បើ Hub ផ្លាស់)។
