...
CI runs `make check-all`, builds the lab Dockerfiles, smoke-tests labs 04, 05, 09, 12, and 13 (`make smoke`), runs concept-lab helpers (`make smoke-concept`), and fails on unfixed CRITICAL findings from Trivy. The Pages workflow also builds the static site on PRs/`develop` and deploys only from `main`. Keep secrets out of git (`.env` is ignored; commit `.env.example` only).
